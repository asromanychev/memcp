...
## Documents Mirror (`storage/documents`)

- `FileSync::WatcherService` копирует файлы из `documents/` в `storage/documents/`, обновляя `index.json`.
- Поддерживаются текстовые форматы (`.md`, `.mdc`, `.txt`, `.rb`, `.js`, `.sql`, `.json`, `.yml`, `.yaml`).
- Удаление в исходном каталоге приводит к удалению копии и записи в индексе при следующем событии watcher.
